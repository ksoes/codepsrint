...
import io.github.ksoes.common.domain.Role;
...
public class AccountService {
    ...
    * */
    ...
    public void signup(AccountForm form) {
        ...
        User user = new User();
        String initialName = RandomStringUtils.randomAlphanumeric(10);
        user.updateUser(initialName, Role.USER.getCode());
     ...
     * */
    @Transactional
    public void signupAdmin(AccountForm form) {
        if (accountRepository.existsById(form.getId())) {
            throw new IllegalArgumentException("이미 존재하는 아이디 입니다.");
        }

        User user = new User();
        user.updateUser("admin", Role.ADMIN.getCode());
